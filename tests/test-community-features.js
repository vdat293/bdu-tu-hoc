...
assert.match(communityService, /resolvedParentId = String\(rootResult\.rows\[0\]\.id\)/);
assert.match(communityService, /WITH RECURSIVE comment_tree/);
assert.match(communityService, /deleted_count: deletedCount/);
...
assert.match(appJs, /button\.dataset\.communityRoot \|\| button\.dataset\.communityReply/);
assert.match(appJs, /data-comment-root-id/);
assert.doesNotMatch(appJs, /Các phản hồi vẫn được giữ lại/);
