...
try {
  ...
  assert.ok(instructorParas.eq(0).find('w\\:ind').attr('w:left'), 'metadata has dynamic left indent');
  assert.equal(instructorParas.eq(0).find('w\\:tab').attr('w:pos'), instructorParas.eq(0).find('w\\:ind').attr('w:left'), 'metadata tab stop matches left indent for hanging align');
  assert.ok(Number(instructorParas.eq(0).find('w\\:spacing').attr('w:before')) > 0, 'instructor has dynamic before gap');
  const docTypeParas = out.a.body.children('w\\:p').filter((_,e)=>out.a.$(e).text() === 'ĐỒ ÁN TỐT NGHIỆP');
  assert.equal(docTypeParas.eq(0).find('w\\:spacing').attr('w:before'), '240', 'docType has before 12pt (240 dxa)');
  ...
  assert.equal(ornamentParas.eq(0).find('w\\:spacing').attr('w:before'), '120', 'ornament has before 6pt (120 dxa)');
  assert.equal(ornamentParas.eq(0).find('w\\:spacing').attr('w:after'), '160', 'ornament has after 8pt (160 dxa)');
  const dateCoverParas = out.a.body.children('w\\:p').filter((_,e)=>out.a.$(e).text().includes('Thành phố Hồ Chí Minh, tháng'));
  assert.ok(Number(dateCoverParas.eq(0).find('w\\:spacing').attr('w:before')) > 0, 'cover date line has dynamic before gap');
